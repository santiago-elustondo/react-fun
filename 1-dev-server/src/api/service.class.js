...
// @todo: move to utils
const doAsync = (fn, ms = 0) => setTimeout(fn, ms)

export class ApiService {

  constructor() {
    this.user = JSON.parse(localStorage.getItem('thinker-user'))
  ...
  async tokenIsValid() {
    ...
    return (
      ...
      validateResponse.body._id === this.user._id 
    )
  }
      ...
      this._commentWatchers[thoughtId] = {
        ...
        interval: setInterval(async () => {
          ...
          // if (commentsResponse.response.ok) {
            ...
            w.handlers.forEach(h => doAsync(() => h(w.currentVal)))
          // } else {
          //   w.errHandlers.forEach(h => doAsync(() => h(commentsResponse.response.code)))
          // }
